...
export default function Sale() {
  ...
  const addItem = () => { setItems(p => [...p, { ...emptySaleItem }]); };
  ...
  const openEdit = async (sale) => {
    ...
    try {
      ...
      const mappedItems = await Promise.all(r.data.items.map(async (it) => {
        ...
        return { ...emptySaleItem, product_id: it.product_id, product_name: it.product_name, pack_size: it.pack_size || '', batch_no: it.batch_no || '', sale_rate: it.sale_rate, qty: it.qty, bonus: it.bonus || 0, discount_pct: it.discount_pct || 0, tax_pct: it.tax_pct || 0, total: it.total, _batches: batches };
      }));
    ...
    <>
      <div className="form-grid form-grid-4" style={{ marginBottom: 20 }}>
        <div className="form-group" style={{ margin: 0 }}>
          ...
          <select className="form-control" value={header.customer_id} onChange={e => setHeader(p => ({ ...p, customer_id: e.target.value }))}>
      ...
      {items.map((item, idx) => (
        <div key={idx} style={{
          ...
          <select className="form-control" style={colStyle} value={item.product_id}
          ...
          <input className="form-control" type="number" style={colStyle} placeholder="0"
            value={item.bonus} onChange={e => updateItem(idx, 'bonus', e.target.value)} />
          <input className="form-control" type="number" step="0.01" style={colStyle} placeholder="0%"
